...
To allow `zeitwerk` to work properly despite `grpc_tools_ruby_protoc` not respecting the naming conventions, by default the task will create an initializer that requires the generated stubs and stops the specified folder from being autoloaded (if it's inside the `app` directory).
